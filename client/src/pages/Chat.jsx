...
.sidebar{width:290px;flex-shrink:0;background:var(--bg1);border-right:1px solid var(--border);display:flex;flex-direction:column;position:relative;z-index:1}
.sidebar-top{padding:20px 16px 14px;border-bottom:1px solid var(--border);background:linear-gradient(180deg,var(--bg2) 0%,var(--bg1) 100%)}
...
.tabs{display:flex;gap:6px;background:var(--bg0);border-radius:10px;padding:4px}
.tab{flex:1;padding:8px 12px;border:none;border-radius:8px;color:var(--text2);font-size:12px;font-weight:500;font-family:'Outfit',sans-serif;cursor:pointer;background:transparent;transition:all .2s ease}
.tab.on{background:var(--accent);color:#fff;box-shadow:0 4px 12px rgba(124,58,237,.4)}
.tab:not(.on):hover{background:var(--bg3);color:var(--text1)}
.search-wrap{padding:10px 14px 8px;position:relative}
.search-wrap input{width:100%;background:var(--bg2);border:1px solid var(--border);border-radius:10px;padding:8px 10px 8px 12px;color:var(--text0);font-size:13px;font-family:'Outfit',sans-serif;outline:none;transition:border-color .2s}
...
.contact{display:flex;align-items:center;gap:10px;padding:10px;border-radius:12px;cursor:pointer;transition:all .2s ease;margin-bottom:2px;position:relative;animation:fadeSlideUp .25s ease both}
.contact:hover{background:var(--bg3)}
.contact.active{background:linear-gradient(135deg,rgba(124,58,237,.18),rgba(168,85,247,.08));border:1px solid rgba(124,58,237,.25)}
...
.gpanel{background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:12px;margin-bottom:8px;animation:fadeSlideUp .2s ease}
.ginput{width:100%;background:var(--bg0);border:1px solid var(--border);border-radius:8px;padding:8px 10px;color:var(--text0);font-size:12px;font-family:'Outfit',sans-serif;outline:none;margin-bottom:8px}
.gcreate{width:100%;padding:8px;background:linear-gradient(135deg,var(--accent),var(--accent2));border:none;border-radius:8px;color:#fff;font-size:12px;font-family:'Outfit',sans-serif;cursor:pointer;font-weight:500;transition:opacity .2s}
.gcreate:hover{opacity:.85}
.gnewbtn{width:100%;padding:9px;background:transparent;border:1px dashed var(--border);border-radius:10px;color:var(--text2);font-size:12px;font-family:'Outfit',sans-serif;cursor:pointer;margin-bottom:8px;transition:all .2s}
.gnewbtn:hover{border-color:var(--accent);color:var(--accent3)}
...
.chat-head{padding:14px 24px;display:flex;align-items:center;gap:14px;background:var(--bg1);border-bottom:1px solid var(--border);position:relative;animation:fadeIn .3s ease}
...
.msgs{flex:1;overflow-y:auto;padding:24px 28px;display:flex;flex-direction:column;gap:6px;background:radial-gradient(ellipse 80% 60% at 50% 0%,rgba(124,58,237,.04) 0%,transparent 60%)}
...
.bubble{max-width:62%;padding:11px 15px;font-size:14px;line-height:1.55;position:relative;word-break:break-word;transition:transform .1s ease}
...
.mine .bubble{background:linear-gradient(135deg,#7C3AED,#A855F7);color:#fff;border-radius:18px 18px 4px 18px;box-shadow:0 4px 20px rgba(124,58,237,.35)}
.theirs .bubble{background:var(--bg2);color:var(--text0);border:1px solid var(--border);border-radius:18px 18px 18px 4px}
...
.footer{display:flex;flex-direction:column;background:var(--bg1);border-top:1px solid var(--border)}
...
.ibtn{width:40px;height:40px;border-radius:12px;background:var(--bg2);border:1px solid var(--border);color:var(--text2);display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:18px;transition:all .2s;flex-shrink:0}
.ibtn:hover{border-color:var(--accent3);color:var(--accent3);background:rgba(124,58,237,.08);transform:scale(1.05)}
.msginput{flex:1;background:var(--bg2);border:1px solid var(--border);border-radius:14px;padding:11px 18px;color:var(--text0);font-family:'Outfit',sans-serif;font-size:14px;outline:none;transition:all .2s}
.msginput:focus{border-color:rgba(124,58,237,.5);box-shadow:0 0 0 3px rgba(124,58,237,.1)}
...
.empty{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;color:var(--text2);animation:fadeIn .5s ease}
.empty-icon{font-size:52px;filter:drop-shadow(0 0 20px rgba(124,58,237,.3))}
...
.call-card { background: var(--bg1); border: 1px solid var(--border); padding: 32px; border-radius: 20px; display: flex; flex-direction: column; align-items: center; gap: 20px; box-shadow: 0 20px 40px rgba(0,0,0,0.5); text-align: center; max-width: 380px; width: 90%; }
...
.call-btn.accept { background: var(--green); color: white; box-shadow: 0 4px 14px rgba(16,185,129,0.4); }
.call-btn.decline { background: var(--red); color: white; box-shadow: 0 4px 14px rgba(239,68,68,0.4); }
...
.remote-video-wrap { flex: 1; background: #020208; border-radius: 16px; border: 1px solid var(--border); overflow: hidden; position: relative; }
.remote-video-wrap video { width: 100%; height: 100%; object-fit: cover; }
.local-video-wrap { width: 220px; aspect-ratio: 4/3; background: #090915; border-radius: 12px; border: 2px solid var(--accent); overflow: hidden; position: absolute; bottom: 16px; right: 16px; box-shadow: 0 8px 24px rgba(0,0,0,0.6); z-index: 10; }
...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API_URL}/api/auth/users`).then(async (res) => {
      ...
      for (const u of filtered) {
        try {
          const r = await axios.get(`${API_URL}/api/messages?sender=${u.name}&receiver=${currentUser.name}`);
    ...
    axios.get(`${API_URL}/api/groups?username=${currentUser.name}`).then(r => setGroups(r.data));
  }, [currentUser]);
  ...
  useEffect(() => {
    ...
    axios.get(`${API_URL}/api/messages?sender=${currentUser.name}&receiver=${selectedUser.name}`).then(async (res) => {
      setMessages(res.data);
      await axios.put(`${API_URL}/api/messages/mark-seen`, { sender: selectedUser.name, receiver: currentUser.name });
      socket.emit("message_seen", { sender: selectedUser.name, receiver: currentUser.name });
    });
  }, [selectedUser]);
  ...
  useEffect(() => {
    ...
    socket.on("receive_message", async (data) => {
      if (selectedUser && (data.sender === selectedUser.name || data.receiver === selectedUser.name)) {
        ...
        if (data.sender === selectedUser.name) {
          await axios.put(`${API_URL}/api/messages/mark-seen`, { sender: data.sender, receiver: currentUser.name });
          socket.emit("message_seen", { sender: data.sender, receiver: currentUser.name });
        }
  ...
  return (
    <div className="app">
      ...
      {/* WEBRTC FULL-SCREEN INTERACTIVE MATRIX CALL SYSTEM OVERLAY */}
      {callingState !== "idle" && (
        <div className="call-overlay">
          {callingState === "ringing" && isIncomingCall ? (
            <div className="call-card">
              <div style={{fontSize: 54}}>📞</div>
              <h2 style={{color: "var(--text0)"}}>{callerName}</h2>
              <p style={{color: "var(--text2)", fontSize: 13}}>Incoming {callType} call handshake payload requested...</p>
              <div className="call-actions">
                <button className="call-btn accept" onClick={answerIncomingCall}>Accept</button>
                <button className="call-btn decline" onClick={rejectOrHangupCall}>Reject</button>
              </div>
            </div>
          ) : (
            <>
              <h2 style={{ color: "var(--text0)", marginBottom: "20px", fontWeight: 500, letterSpacing: 0.5 }}>
                {callingState === "calling" && `Dialing out tracking frequency link to ${selectedUser?.name}...`}
                {callingState === "connected" && `Secured WebRTC Peer Stream Active`}
              </h2>

              <div className="video-grid" style={{ justifyContent: callType === "audio" ? "center" : "initial", alignItems: "center" }}>
                {callType === "video" ? (
                  <>
                    <div className="remote-video-wrap">
                      <video ref={remoteVideoRef} autoPlay playsInline />
                      {callingState === "calling" && (
                        <div style={{position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center", color:"var(--text2)"}}>
                  ...
                  <div className="call-card" style={{width: 300}}>
                    <div style={{fontSize: 48, animation: "pulse 1.5s infinite"}}>🎙️</div>
                    <h3 style={{color:"var(--text0)"}}>{selectedUser?.name || callerName}</h3>
                    <p style={{color:"var(--text2)", fontSize:12}}>{callingState === "connected" ? "Voice Connected" : "Connecting line payload..."}</p>
                    <audio ref={remoteVideoRef} autoPlay />
                  </div>
                )}
              </div>

              <button className="call-btn decline" style={{width: 140, padding: 14, borderRadius: 30}} onClick={rejectOrHangupCall}>
                Hang Up
              </button>
            </>
          )}
      ...
      {activeContact ? (
        <div className="chat-area">
          <div className="chat-head">
            ...
            {selectedUser && callingState === "idle" && (
